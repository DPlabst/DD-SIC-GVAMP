...
for modstr = ["ASK-0.2"]
    for M = [16]
        [filename, vver] = gvamp( ...
            ...
            Ptx = '17.5', ... % Transmit powers in dB, e.g.,'-10:2:50' (mind the SNR definition)
            ...
            RxBW = 1, ... % [0/1]: PD bandwidth limit to signal bandwidth
            ...
            dampcfgStepW = [10, 10, 1], ... [minWindow, maxWindow, [0/1]:reduce step window with increasing SIC stage]
            annealcfg = {[100, 1], [0.28, 0.19, 1], [0]}, ... % Cell-array {[n_it_thresh, reduce threshold vs. SIC stage], [alpha, beta, minAnneal], [anneal if iter > switchIterThresh]}
