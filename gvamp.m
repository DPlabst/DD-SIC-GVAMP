...
function [filename, vver] = gvamp(nvArgs)
    arguments
        ...
        nvArgs.nu (1, :) double = [1, 0] %Noise variances of [optical white CSCG, electrical real WGN].
        ...
        nvArgs.RxBW logical = false % [0/1]: PD bandwidth limit to signal bandwidth
        ...
        nvArgs.saveFile double = 0 % Save results to file
    end
    ...
    rng(1); % set rng
    [P, p_vec] = gen_prec(nvArgs.ch, Pcfg, N); % Generate precoder
    ...
    for ind_sic = s_vec %Loop over SIC stages
        ...
        % ---------- Compute Adata ------------
        [idx_data, idx_pil, Adata, Apil, Nprime] = gen_sic_mat(N_pil, N_span, N, A, ind_sic, S_SIC);
        trAAh = sum(conj(A) .* A, 'all'); %Energy of (full) A
        trAdAdh = sum(conj(Adata) .* Adata, 'all'); %Energy of Adata
        ...
        if abs(3 - 10 * log10(1 / N * trAAh)) > 0.2; error('Power constraint violation'); end %Sanity check

        fprintf('\n')
        fprintf('Starting iterations ...\n')

        for ind_n_x_rb = 1:(L_SNR * N_block * N_recov) %(par)for over all SNRs x number of blocks x number of trials per block

            w_star = [];
            [ind_n, rb, rr] = ind2sub([L_SNR, N_block, N_recov], ind_n_x_rb);
            rng(rb + 123456); % set seed
            ...
            s_intf = u0(idx_pil).'; %Known interference or pilots
            w_intf = Apil * s_intf; % Known interference via pilots, SIC stages

            P_tx_sym_measured(ind_n_x_rb) = P_tx_sym; %Measure power

            %% -------- VAMP ---------
            w_nonoise = A * (u0).'; %Apply channel
            ...
            y_noisy = downsample(y_inter, 2); %Downsample (may give aliasing depending on alpha of the PS filter)

            n_rx = sqrt(RXnoise.var_n_post) * randn(N_os * N, 1); %Generate real white post-intensity GN
            y_noisy = y_noisy + n_rx; %Add post-intensity noise

            % -------------- GVAMP Initial messages ---------
            rng(rr + 2 * 123456); %Different initializations
            ...
            hnd_lmmse_u2w2 = @(r2, nu_U2, p2, nu_W2, iter, flag_trig, rGVAMP, u2d_hat_part) ...
                lmmse_u2w2(Pcfg, r2, nu_U2, p2, nu_W2, w_intf, N, Nprime, idx_data, L_A, p_vec, n_it_thresh, ...
                iter, TXnoise.var_n_pre, Lambda_D, gam_BdBsam, N_os, ind_sic, P_tx_vec_dB(ind_n), nvArgs.annealcfg, Adata, rGVAMP, flag_trig, u2d_hat_part);
            % ----------------

            %Handle for damp cost function
            hnd_damp_cost = @(r1, nu_X1, x1, alpha1) calc_damp_cost(y_noisy.', N, r1, nu_X1, x1, alpha1, TXnoise.var_n_pre, gam_BdBsam, nu_p_vec, pU, Adata, w_intf, trAdAdh, idx_data, L_A, p_vec, Pcfg, N_os, nvArgs.rGVAMP, hnd_denoise_u1);
            ...
            if nvArgs.runGVAMP
                ...
                [IqYX, SER, NMSE, IqYX_vamp_iter, NMSE_vamp_iter, APP_cost, reqIter, u1_best] = run_gvamp_asym( ...
                    nu_p_vec, ...
                    phat.', ...
                    pvar.', ...
                    rhat.', ...
    ...
    if nvArgs.saveFile
        ...
        clear s_intf rhat F arg_rnd simWDM nvArgs p_vec h_steep_convd h_tx h_tx_lim  h_comb_pad a_comb
        clear alpha1_SEs alpha2_SEs beta1_SEs beta2_SEs rate_SEs z_star x_star
        clear D1 D2 Fh Fh_A Fh_B Fh_C Fh_D Fn
        clear P_tx_sym_measured P_tx_wave_measured
        clear APP_cost_ten  HDMI_ten reqiter_VAMP_lvec REQITER_ten init_cfg GZF
        clear *_iter* *_lvec* RXnoise minRR ps_filter cluster P_tx_sym_measured_ten P_tx_wave_measured_ten autoDampCFG
